...
/**
 * Taken from IANA TLS Parameters table
 * https://www.iana.org/assignments/tls-parameters/tls-parameters.xhtml
 */
const tlsCipherTable = {
...
/**
 * @param {string} cipID TLS cipher identifier in hexadecimal string.
 */
const findTLSCipherName = (cipID) => {
    if (!cipID || typeof cipID !== "string" || cipID === "" || cipID?.length > 4) return "FATAL_CIPHER_NOT_FOUND";
    cipID = cipID.toUpperCase();
    if (cipID.length < 4) cipID = cipID.padStart(4, "0");

    return tlsCipherTable[cipID] || "FATAL_CIPHER_NOT_FOUND";
};
...
/**
 * @param {string} sigSchID Signature scheme identifier in hexadecimal string.
 */
const findTLSSignatureSchemeName = (sigSchID) => {
    if (!sigSchID || typeof sigSchID !== "string" || sigSchID === "" || sigSchID?.length > 4) return "fatal_scheme_not_found";
    sigSchID = sigSchID.toUpperCase();
    if (sigSchID.length < 4) sigSchID = sigSchID.padStart(4, "0");

    return tlsSignatureSchemeTable[sigSchID] || "fatal_scheme_not_found";
};
...
export {
    ...
    tlsCipherTable,
    findTLSCipherName,
    tlsSignatureSchemeTable,
    findTLSSignatureSchemeName,
};
